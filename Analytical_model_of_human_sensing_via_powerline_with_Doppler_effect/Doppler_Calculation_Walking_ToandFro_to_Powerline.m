% --------------------------------------------------------------------------------------------------
%
%    Analytical Model of Human Walking To and Fro Powerline by Calculating Doppler Signature. 
%
...
% position of human when walk vertically along the powerline
Px_human=ones(1,Nt)*H/2;

t1 = 0:dt:0.5;
t2 = 0.5+dt:dt:1.5;
t3 = 1.5+dt:dt:2;
Py_human_1_1 = 4-2*t1.^2;
Py_human_1_2 = 4.5-2*t2;
Py_human_1_3 = 3-t3;
Py_human_1 = [Py_human_1_1,Py_human_1_2,Py_human_1_3];
Py_human_2 = flip(Py_human_1);
Py_human = [Py_human_1,Py_human_2(2:end)];

d=1;                                    % distance between human and y axis
Pz_human=ones(1,Nt)*d;

% velocity of human
Vx = zeros(1,length(Px_human));

Vy_1_1 = -4*t1;
Vy_1_2 = ones(1,length(Py_human_1_2))*Vy_max;
Vy_1_3 = 4*t3-8;
Vy_1 = [Vy_1_1,Vy_1_2,Vy_1_3];
Vy_2 = -flip(Vy_1);
Vy = [Vy_1,Vy_2(2:end)];

Vz = zeros(1,length(Pz_human));
